...
// =============================================================================
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('[data-currency-code]').forEach(item => {
            item.addEventListener('click', (event) => {
                ...
                const newSymbol = event.target.dataset.currencySymbol;
                const newCode = event.target.dataset.currencyCode;
                currentCurrency = { symbol: newSymbol, code: newCode };
                localStorage.setItem('currentCurrency', JSON.stringify(currentCurrency));
                updateCurrencyDisplay();
                // Reload current tab to reflect currency change
        ...
        let authToken = null;
    let currentCurrency = JSON.parse(localStorage.getItem('currentCurrency')) || { symbol: '€', code: 'EUR' };

    const updateCurrencyDisplay = () => {
        const currencySelectorButton = document.getElementById('currency-selector');
        if (currencySelectorButton) {
            currencySelectorButton.textContent = `${currentCurrency.symbol} ${currentCurrency.code}`;
        }
    };

    // --- UTILS --- 
    const formatCurrency = (amount) => {
        return `${amount.toFixed(2)} ${currentCurrency.symbol}`;
    };
    ...
    async function loadDashboardTab() {
        ...
        mainContent.innerHTML = `
            ...
            <div class="row">
                <div class="col-md-3 mb-3"><div class="card text-white bg-primary"><div class="card-header">Chiffre d'Affaires (Aujourd'hui)</div><div class="card-body"><h5 class="card-title">${formatCurrency(kpis.ca_today)}</h5></div></div></div>
                ...
                <div class="col-md-3 mb-3"><div class="card text-dark bg-warning"><div class="card-header">Valeur Totale du Stock</div><div class="card-body"><h5 class="card-title">${formatCurrency(kpis.total_stock_valeur)}</h5></div></div></div>
            </div>
            <div class="row mt-4">
    ...
    async function loadStockTab() {
        ...
        produits.forEach(p => {
            ...
            row.innerHTML = `<td>${p.nom}</td><td>${formatCurrency(p.prix_achat)}</td><td>${formatCurrency(p.prix_vente)}</td><td>${p.quantite}</td><td><button class="btn btn-sm btn-warning edit-btn" data-id="${p.id}"><i class="bi bi-pencil-square"></i></button> <button class="btn btn-sm btn-danger delete-btn" data-id="${p.id}"><i class="bi bi-trash"></i></button></td>`;
        });
    ...
    async function loadVentesTab() {
        ...
        ventes.forEach(v => {
            const row = tableBody.insertRow();
            row.innerHTML = `<td>${v.produit.nom}</td><td>${v.quantite}</td><td>${formatCurrency(v.prix_total)}</td><td>${new Date(v.date).toLocaleString('fr-FR')}</td><td><button class="btn btn-sm btn-warning edit-vente-btn" data-id="${v.id}"><i class="bi bi-pencil-square"></i></button> <button class="btn btn-sm btn-danger delete-vente-btn" data-id="${v.id}"><i class="bi bi-trash"></i></button></td>`;
        });
    ...
    async function loadAnalyseTab() {
        ...
        async function runAnalysis() {
            ...
            document.getElementById('ca-value').textContent = formatCurrency(data.chiffre_affaires);
            document.getElementById('cogs-value').textContent = formatCurrency(data.cogs);
            document.getElementById('depenses-value').textContent = formatCurrency(data.depenses);
            document.getElementById('benefice-net-value').textContent = formatCurrency(data.benefice_net);
    ...
    async function loadFraisTab() {
        ...
        frais.forEach(f => {
            const row = tableBody.insertRow();
            row.innerHTML = `<td>${f.produit.nom}</td><td>${f.description}</td><td>${formatCurrency(f.montant)}</td><td>${new Date(f.date).toLocaleString('fr-FR')}</td><td><button class="btn btn-sm btn-warning edit-frais-btn" data-id="${f.id}"><i class="bi bi-pencil-square"></i></button> <button class="btn btn-sm btn-danger delete-frais-btn" data-id="${f.id}"><i class="bi bi-trash"></i></button></td>`;
        });
    }
